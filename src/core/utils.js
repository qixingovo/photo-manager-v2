// utils.js — ES module version of common.js shared utilities
// v=2
...
export function safeBigint(val, fallback) {
  if (val === null || val === undefined) return fallback;
  var n = parseInt(val, 10);
  return isFinite(n) ? n : fallback;
}

export function highlightKeywords(text, searchValue) {
...
export function getCategoryAndChildrenIds(categoryId, categories) {
  var strId = String(categoryId);
  var ids = [strId];
  (categories || []).forEach(function (c) {
    if (String(c.parent_id) === strId) {
      ids.push.apply(ids, getCategoryAndChildrenIds(c.id, categories));
    }
  });
  return ids;
}

export function getCategoryPath(categoryId, categories, field) {
  var path = [];
...
// 心情表情（共享常量）
export var MOOD_EMOJIS = ['😊', '😢', '😡', '😴', '🥰', '😰', '🤩', '😤'];

// 伴侣喜好档案默认模板（共享常量）
export var DEFAULT_PROFILE = {
...
// 经期忌口清单（共享常量）
export var DIETARY_RESTRICTIONS = ['冰的', '辣的', '咖啡', '酒', '生冷', '油炸', '奶茶'];
